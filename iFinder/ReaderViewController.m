...
- (void)viewDidLoad
{
    ...
    pdfOptions = [[UIActionSheet alloc] init];
    pdfOptions.actionSheetStyle = UIActionSheetStyleBlackOpaque;
    pdfOptions.delegate = self;
    pdfOptions.cancelButtonIndex = 3;
    [pdfOptions addButtonWithTitle: NSLocalizedString(@"Print",@"Print")];
    [pdfOptions addButtonWithTitle: NSLocalizedString(@"Bookmark File",@"Bookmark File")];
    [pdfOptions addButtonWithTitle: NSLocalizedString(@"Open In...",@"Open In...")];
    [pdfOptions addButtonWithTitle: NSLocalizedString(@"Cancel",@"Cancel")];
    options = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(pdfoptions)];
	...
	pagebarRect.size.height = TOOLBAR_WIDTH;
	pagebarRect.origin.y = (viewRect.size.height - TOOLBAR_WIDTH);
		...
		if (index == 0) {
            ...
            if ((printInteractionController != nil) && [printInteractionController isPrintingAvailable])
            {
                ...
                if ([printInteractionController canPrintURL:fileURL] == YES)
                {
                    ...
                    [printInteraction presentFromBarButtonItem:options animated:YES completionHandler:
            ...
            newBookmark = [[MFNewBookmark alloc] initWithPath:[[NSString stringWithFormat:@"%@", path] stringByReplacingOccurrencesOfString:@"file://localhost" withString:@""]];
            [newBookmark presentFrom: self];
            
        } else if (index == 2) {
            NSURL *path = [NSURL URLWithString:[NSString stringWithFormat:@"/private/var/mobile/%@", [document.fileURL lastPathComponent]]];
...
- (void)scrollViewTouchesBegan:(UIScrollView *)scrollView touches:(NSSet *)touches
{
	...
	if ((mainPagebar.hidden == NO))
	{
		if (touches.count == 1) // Single touches only
		{
			...
			CGRect areaRect = CGRectInset(self.view.bounds, PAGING_AREA_WIDTH, TOOLBAR_WIDTH);
...
- (void)handleDoubleTap:(UITapGestureRecognizer *)recognizer
{
	...
	if (recognizer.state == UIGestureRecognizerStateRecognized)
	{
		...
		CGRect zoomArea = CGRectInset(viewRect, PAGING_AREA_WIDTH, TOOLBAR_WIDTH);
...
- (void) pdfoptions {
    seteuid(501);
    pdfOptions.title = [[[NSUserDefaults standardUserDefaults] stringForKey:@"LastFileAccesed"] lastPathComponent];
    seteuid(0);
    [pdfOptions showInView: self.view];
}

- (void)documentInteractionController:(UIDocumentInteractionController *)controller willBeginSendingToApplication:(NSString *)application {
    seteuid(501);
    char *copy = [[NSString stringWithFormat:@"cp -r '%@' /private/var/mobile/", [[NSUserDefaults standardUserDefaults] stringForKey:@"LastFileAccesed"]] UTF8String];
    seteuid(0);
    system(copy);
}
