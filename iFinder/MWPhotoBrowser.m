...
- (void)viewDidLoad {
    ...
    //pagingScrollView.backgroundColor = [UIColor colorWithPatternImage: [UIImage imageNamed: @"imagebg.png"]];
...
//    if (photos.count > 1) {
        ...
        UIBarButtonItem *print = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"Printer.png"] style:UIBarButtonItemStylePlain target:self action:@selector(print)];
        UIBarButtonItem *openin = [[UIBarButtonItem alloc] initWithBarButtonSystemItem: UIBarButtonSystemItemAction target:self action:@selector(open)];
        UIBarButtonItem *savepict = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemOrganize target:self action:@selector(savepict)];
        UIBarButtonItem *trash = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemTrash target:self action:@selector(deleteimg)];
...
-(void) print{
    ...
    UIImageView *imageView = [[UIImageView alloc] initWithImage:[self imageAtIndex:currentPageIndex]];
...
-(void) open{
    NSURL *path = [NSURL fileURLWithPath:[NSString stringWithFormat:@"/private/var/mobile/photo.png"]];
...
-(void) deleteimg{
    char *deletes = [[NSString stringWithFormat: @"rm -rf '%@'", [[deletearray objectAtIndex:currentPageIndex] fullPath]] UTF8String];
    system(deletes);
...
-(void)saved{
    UIAlertView *saveimage = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Image Saved",@"Image Saved") message:[NSString stringWithFormat:@"'%@' %@", [[deletearray objectAtIndex:currentPageIndex] name], NSLocalizedString(@"was succesfully saved to Camera Roll", @"was succesfully saved to Camera Roll")] delegate:self cancelButtonTitle:nil otherButtonTitles:NSLocalizedString(@"OK",@"OK"), nil];
    [saveimage show];
    [saveimage release];
}

- (void) bookmark {
    newbookmark = [[MFNewBookmark alloc] initWithPath:[[deletearray objectAtIndex:currentPageIndex] fullPath]];
    [newbookmark presentFrom: self];
}

- (void)documentInteractionController:(UIDocumentInteractionController *)controller willBeginSendingToApplication:(NSString *)application {
    UIImage *img = [self imageAtIndex:currentPageIndex];
    NSData *imageData = UIImagePNGRepresentation(img);
    [imageData writeToFile:@"/private/var/mobile/photo.png" atomically:NO];	
}
