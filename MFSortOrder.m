...
{
    seteuid(501);
    selected = [[NSUserDefaults standardUserDefaults] integerForKey:@"SortOrder"];
    seteuid(0);
}

- (id) init {
    self = [super init];
    self.title = NSLocalizedString(@"Sort Order", @"Sort Order");
...
- (NSString *) tableView: (UITableView *) theTableView titleForHeaderInSection: (int) section {
	...
	if (section == 0) {
		title = NSLocalizedString(@"Select Sort Order", @"Select Sort Order");
	}
...
- (UITableViewCell *) tableView: (UITableView *) theTableView cellForRowAtIndexPath: (NSIndexPath *) indexPath {
	...
	if (indexPath.section == 0) {
		...
		if (indexPath.row == 0) {
			...
			cell.text = NSLocalizedString(@"Ascending", @"Ascending");
            if (selected == 0){
			...
			cell.text = NSLocalizedString(@"Descending", @"Descending");
            if (selected == 1){
                cell.accessoryType = UITableViewCellAccessoryCheckmark;
            } else {
                cell.accessoryType = UITableViewCellAccessoryNone;
...
- (void) tableView: (UITableView *) theTableView didSelectRowAtIndexPath: (NSIndexPath *) indexPath {
    ...
    [prefs setInteger:indexPath.row forKey:@"SortOrder"];
    [prefs synchronize];
    seteuid(0);
