...
- (id) init {
	...
	self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem: UIBarButtonSystemItemOrganize target: self action: @selector(loadParent)] autorelease];
	...
	operation.title = @"File tasks";
	operation.delegate = self;
	operation.actionSheetStyle = UIBarStyleBlack;
	[operation addButtonWithTitle: @"Load directory"];
	[operation addButtonWithTitle: @"Download file"];
	[operation addButtonWithTitle: @"Cancel"];
...
- (UITableViewCell *) tableView: (UITableView *) theTableView cellForRowAtIndexPath: (NSIndexPath *) indexPath {
	...
	if (cell == nil) {
		cell = [[UITableViewCell alloc] initWithStyle: 3 reuseIdentifier: @"MFSFTPCell"];
	}
	
	cell.textLabel.text = [[files objectAtIndex: indexPath.row] name];
	cell.detailTextLabel.text = [NSString stringWithFormat: @"Size: %@; mode: %@", [[files objectAtIndex: indexPath.row] fsize], [[files objectAtIndex: indexPath.row] permissions]];
