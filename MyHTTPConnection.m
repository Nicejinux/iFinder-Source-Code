...
{    
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]){
        ...
        [outdata appendFormat:@"<title>iFinder</title>", server.name];
        [outdata appendFormat:@"<link rel='stylesheet' type='text/css' href='/Applications/iFinder.app/webstyles.css' /></head><link rel='icon' href='/Applications/iFinder.app/Icon114.png' type='image/png'>", server.documentRoot];
        ...
        [outdata appendFormat:@"<h1>There is no folder at path: %@</h1>", path];
    ...
    //[outdata appendString:@"<style>html {background-color:#eeeeee} body { background-color:#FFFFFF; font-family:Tahoma,Arial,Helvetica,sans-serif; font-size:18x; margin-left:15%; margin-right:15%; border:3px groove #006600; padding:15px; } </style>"];
    [outdata appendString:@"</head><body>"];
	[outdata appendFormat:@"<h1>Folder: %@</h1>", [path stringByReplacingOccurrencesOfString:@"//" withString:@"/"]];
    [outdata appendString:@"<table cellpadding=\"0\">\n<tr><th class=\"fileicon\"></th><th class=\"filename\">Name</th><th class=\"filedate\">Modification Date</th><th class=\"filesize\">Size</th></tr>"];
    if (![path isEqualToString:@"/"]){
        [outdata appendFormat:@"<tr class=\"even\"><td class=\"fileicon\"></td><td><a href=\"..\">Parent Directory</a></td>\n<td>&nbsp;%s</td><td>&nbsp;&nbsp;%s</td></tr>\n", "-", "-"];
    }
    for (NSString *fname in array)
    {
        ...
        NSString *class;
        if ([array indexOfObject:fname] % 2){
            class = [path isEqualToString:@"/"] ? @"odd" : @"even";
        } else {
            class = [path isEqualToString:@"/"] ? @"even" : @"odd";
        }
		//NSLog(@"fileDict: %@", fileDict);
        NSString *modDate = [[fileDict objectForKey:NSFileModificationDate] description];
        NSString *fileiconstring;
        NSString *fullname;
		
        mffile = [[MFFile alloc] init];
        mffile.path = [path stringByAppendingFormat:@"%@", fname];
        mffile.isSymlink = (readlink([[[path stringByReplacingOccurrencesOfString:@"//" withString:@"/"] stringByAppendingString:fname] UTF8String], NULL, 0) != -1);
        NSString *pathurl;
        NSString *filesize;
        if (mffile.isSymlink){
            char *buf = malloc (256 * sizeof (char));
            int bytes = readlink ([[path stringByAppendingString:fname] UTF8String], buf, 255);
            buf[bytes] = '\0';
            if ([[[NSString stringWithUTF8String: buf] substringWithRange:NSMakeRange(0, 1)] isEqualToString:@"/"]) {
                pathurl = [NSString stringWithUTF8String: buf];
            }else{
                pathurl = [NSString stringWithFormat:@"/%@", [NSString stringWithUTF8String: buf]];
            }
            free (buf);
            
            NSDictionary *fileDict2 = [[NSFileManager defaultManager] fileAttributesAtPath:pathurl traverseLink:NO];
            if ([[fileDict2 objectForKey:NSFileType] isEqualToString: @"NSFileTypeDirectory"]){
                pathurl = [fname stringByAppendingString:@"/"];
                fileiconstring = @"/Applications/iFinder.app/dir.png";
                filesize = @"--";
            } else {
                fileiconstring = @"/Applications/iFinder.app/file.png";            
                filesize = @"--";
            }
            ...
            if ([[fileDict objectForKey:NSFileType] isEqualToString: @"NSFileTypeDirectory"]){
                ...
                fileiconstring = @"/Applications/iFinder.app/file.png";            
                filesize = [NSString stringWithFormat:@"%8.1f kB", [[fileDict objectForKey:NSFileSize] floatValue] / 1024];
            }
        }
		//[outdata appendFormat:@"<a href=\"%@\">%@</a>		(%8.1f Kb, %@)<br />\n", fname, fname, [[fileDict objectForKey:NSFileSize] floatValue] / 1024, modDate, fname];
        if ([modDate isEqualToString:@"(null)"]){
            modDate = @"-";
        }
        
        /*NSDateFormatter *dateFormat2 = [[NSDateFormatter alloc] init];
        [dateFormat2 setDateFormat:@"%d %B %Y"];
        NSString *dateString = [dateFormat2 stringFromDate:date];*/
        
        [outdata appendFormat:@"<tr class=\"%@\"><td class=\"fileicon\"><img src=\"%@\" width=16 height=16/></td><td><a href=\"%@\">%@</a></td>\n<td>&nbsp;%@</td><td>&nbsp;&nbsp;%@</td></tr>\n", class,fileiconstring, pathurl, fname, [modDate stringByReplacingOccurrencesOfString:@"+0000" withString:@""], filesize];
        
//        [[fileDict objectForKey:NSFileType] isEqualToString: @"NSFileTypeDirectory"] ? "dir" : "file"
    ...
    if ([self supportsPOST:path withSize:0])
	{
		...
		[outdata appendString:@"</label></br>"];
		[outdata appendString:@"<label>"];
        [outdata appendFormat:@"<input type=\"hidden\" name=\"url\" value=\"%@\">", [path stringByReplacingOccurrencesOfString:@"//" withString:@"/"]];
        ...
        for (int i = 0; i < [postDataChunk length] - l; i++)
        {
            ...
            if ([[postDataChunk subdataWithRange:searchRange] isEqualToData:separatorData])
            {
                ...
                else
                {
                    ...
                    postInfoComponents = [[postInfoComponents lastObject] componentsSeparatedByString:@"\""];
                    postInfoComponents = [[postInfoComponents objectAtIndex:1] componentsSeparatedByString:@"\\"];
                    NSString *uri = [NSString stringWithFormat:@"%@", [NSMakeCollectable(CFHTTPMessageCopyRequestURL(request)) autorelease]];
                    NSString* filename = [[[[server documentRoot] path] stringByAppendingPathComponent:uri] stringByAppendingPathComponent:[postInfoComponents lastObject]];
