...
+ (NSString *) fileTypeForName: (NSString *) name {
	...
	NSString *type;
	NSString *typeStr = [[[[name componentsSeparatedByString: @"."] lastObject] lowercaseString] copy];
	if ([typeStr isEqualToString: @"png"]) {
		...
		type = @"image";
	} else if ([typeStr isEqualToString: @"ico"]) {
		type = @"image";
		...
		type = @"ppt";
	} else if ([typeStr isEqualToString: @"pps"]) {
		type = @"ppt";
		...
		type = @"database";
	}else if ([typeStr isEqualToString: @"backup"]) {
		type = @"iFinderBackup";
	} else {
		type = @"file";
	}
...
+ (UIImage *) imageForType: (NSString *) type {
    ...
    if ([[NSUserDefaults standardUserDefaults] integerForKey: @"MFTheme"] == 1){
        fileName = [NSString stringWithFormat: @"%@Metallen.png", type];
    } else {
        fileName = [NSString stringWithFormat: @"%@.png", type];
    }
    seteuid(0);
